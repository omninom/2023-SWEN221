...
public class Board {
  ...
  private boolean isRowFull(int y) {
     assert yOk(y) : "row " + y + "not in 0-19";
     return rangeX().allMatch(x-> inner.get(y).get(x) != Color.EMPTY);  //stream on the row and check if all the cells are not empty i.e do not contain Color.EMPTY
   }
  ...
  public void clearFullRows() {
    reverseRangeY()                     //stream on the rows in reverse order
      .filter(y-> isRowFull(y))         //keep only the full rows
      .forEach(y-> clearRow(y));        //clear them
  }


  /**
   * toString method for the board. It represents the board as a string of 20 lines, each line containing 10 cells separated by "|"
   * The way it works is that it streams on the rows in reverse order (starting from the bottom of the board)
   * and for each row it streams on the columns and collects the colors in a string
   * The string is then joined with a "|" separator and the whole thing is joined with a "\n" separator
  ...
  /**
   * Helper method for toString. It prints a single line of the board (a single row)
   * the line is built by streaming on the columns and collecting the colors in a string as explained in the method above
   * finally we collect the string with a "|" separator.
