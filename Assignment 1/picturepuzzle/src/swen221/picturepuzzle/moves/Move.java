...
public class Move implements Operation {
	...
	public static boolean nextTo(Location l1, Location l2) {
		int dx = Math.abs(l1.col - l2.col);		//calculate difference in x and y
		int dy = Math.abs(l1.row - l2.row);
		return (dx == 1 && dy == 0) || (dx == 0 && dy == 1); //return true if either x or y is 1
	}
}
