...
public class Board {
	...
	/**
	 * The initial state of the board, where all cells are in correct positions
	 */
	...
	public Board(int[] image, int width, int dimension) {
		...
		this.complete = this.toString();	// set the complete variable to the initial state of the board, where all cells are in correct positions
	}
	...
	public boolean isFinished() {
		return this.toString().equals(complete);	//check if current state of board is equal to the initial state of the board
	}
	...
	public void swap(Location l1, Location l2) {
		Cell temp = getCellAt(l1);	//store first cell
		cells[(l1.row * dimension) + l1.col] = getCellAt(l2);	//replace first cell with second cell
		cells[(l2.row * dimension) + l2.col] = temp;			//set the second cell to the first cell
	}
